using DeconvOptim: gradient, plan_conv, get_plan, fft_or_rfft, p_conv_aux!, deconvolution
using FFTW

"""
    deconvRL(measured, psf; regularizer=nothing, λ=0.05, iterations=100, conv_dims=1:ndims(psf), threshold=0, progress=nothing)
...
"""
    deconvRL(measured, psf, psf_bp; regularizer=nothing, λ=0.05, iterations=100, conv_dims=1:ndims(psf), threshold=0, progress=nothing)

    Classical iterative Richardson-Lucy iteration scheme for deconvolution.
    `measured` is the measured array and `psf` the point spread function.
    `psf_bp` is the psf for backpropagation.
...
function deconvRL(
    ...
    if shift
        psf = ifftshift(psf)  # Shift the PSF to the center of the array
        psf_bp = ifftshift(psf_bp)  # Shift the PSF for backpropagation
    end
    
    otf, conv_temp = plan_conv(measured, psf, conv_dims)

    # Initialize otf_conj based on psf_bp
    otf_conj = plan_conv(measured, psf_bp, conv_dims)[1]
...
function BackProjector(PSF_fp; bp_type="traditional", alpha=0.001, beta=1, n=10, resFlag=1, iRes=[0, 0, 0], verboseFlag=false)
    ...
    verboseFlag && println("Back projector type: $bp_type")
    ...
    verboseFlag && println("Resolution cutoff (spatial): $resx x $resy x $resz")
    verboseFlag && println("Resolution cutoff (Fourier): $tx x $ty x $tz")
    ...
    bp_methods = Dict(
        "traditional" => () -> (flippedPSF, fft(ifftshift(flippedPSF))),
        "wiener" => () -> (fftshift(real(ifft(OTF_flip_norm ./ (abs.(OTF_flip_norm).^2 .+ alpha)))), OTF_flip_norm ./ (abs.(OTF_flip_norm).^2 .+ alpha)),
        "gaussian" => () -> begin
            PSF_bp = gen_gaussianPSF(Sx, Sy, Sz, resx, resy, resz, dims)
            (PSF_bp, fft(ifftshift(PSF_bp)))
        end,
        "butterworth" => () -> butterworth_filter(Sx, Sy, Sz, tx, ty, tz, beta, n, dims),
        "wiener-butterworth" => () -> wiener_butterworth_filter(OTF_flip, OTF_abs / OTFmax, alpha, beta, Sx, Sy, Sz, tx, ty, tz, n, dims)
    )

    bp_func = get(bp_methods, bp_type, () -> error("Unsupported bp_type: $bp_type"))
    PSF_bp, OTF_bp = bp_func()
    return Float32.(PSF_bp), OTF_bp
end
